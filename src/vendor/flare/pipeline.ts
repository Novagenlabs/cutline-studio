 ...
 * Artwork geometry.
 *
 * ADAPTED FROM THE UPSTREAM EXAMPLE. Upstream these are three constants
 * derived from the Next.js logo's viewBox — a tall glyph, centred in a padded
 * box. Ours is a wide lockup, so they have to differ, and they are mutable
 * here rather than edited in place so the rest of this file stays diffable
 * against a future `vgpu examples pull`.
 *
 * `setLogoGeometry` is the only addition to this module's API.
 */
let GLYPH_CENTER_IN_BOX: Point = [(48 + 466 / 2) / 514, (88 + 536 / 2) / 624];
let LOGO_HEIGHT_RATIO = 0.62;
let LOGO_ASPECT = 514 / 624;

export function setLogoGeometry(geometry: {
  /** Where the ink sits inside its box, 0–1, so the light can centre on it. */
  readonly centerInBox?: Point;
  /** Logo height as a fraction of the smaller canvas dimension. */
  readonly heightRatio?: number;
  /** Width / height of the rasterised artwork. */
  readonly aspect?: number;
}): void {
  if (geometry.centerInBox) GLYPH_CENTER_IN_BOX = geometry.centerInBox;
  if (geometry.heightRatio !== undefined) LOGO_HEIGHT_RATIO = geometry.heightRatio;
  if (geometry.aspect !== undefined) LOGO_ASPECT = geometry.aspect;
}
